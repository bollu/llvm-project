...
class ScfYieldToRgn : public mlir::ConversionPattern {
public:
    explicit ScfYieldToRgn(mlir::MLIRContext *context)
            : mlir::ConversionPattern(mlir::scf::YieldOp::getOperationName(), 1,
                                      context) {}
    ...
    matchAndRewrite(mlir::Operation *Op, mlir::ArrayRef<mlir::Value> Operands,
        ...
        mlir::scf::YieldOp YieldOp = llvm::cast<mlir::scf::YieldOp>(Op);
        Rewriter.setInsertionPointAfter(YieldOp);

        Rewriter.replaceOpWithNewOp<RgnReturnOp>(YieldOp, YieldOp.getOperands());
        return mlir::success();
    }
};


class ScfIfToRgn : public mlir::ConversionPattern {
public:
    ...
    matchAndRewrite(mlir::Operation *Op, mlir::ArrayRef<mlir::Value> Operands,
        ...
        mlir::scf::IfOp IfOp = llvm::cast<mlir::scf::IfOp>(Op);
        Rewriter.setInsertionPointAfter(Op);

        RgnValOp ThenRgn = Rewriter.create<RgnValOp>(
                IfOp.getLoc(), IfOp->getResultTypes());
        Rewriter.cloneRegionBefore(IfOp.thenRegion(), ThenRgn.getRegion(), ThenRgn.getRegion().end());

        RgnValOp ElseRgn = Rewriter.create<RgnValOp>(
                IfOp.getLoc(), IfOp->getResultTypes());
        Rewriter.cloneRegionBefore(IfOp.elseRegion(), ElseRgn.getRegion(), ElseRgn.getRegion().end());

        mlir::SelectOp SelectRgn = Rewriter.create<mlir::SelectOp>(IfOp.getLoc(), IfOp.condition(), ThenRgn, ElseRgn);
...
bool runConversion(mlir::Operation *module,
...
struct PatternCallValKnownRegion : public mlir::OpRewritePattern<RgnCallValOp> {
    ...
    matchAndRewrite(RgnCallValOp call,
        ...
        // code from: https://github.com/llvm/llvm-project/blob/9a11c70c1856f4e801d0863c552c754f28110237/mlir/lib/Conversion/SCFToStandard/SCFToStandard.cpp#L411
        ...
        for (mlir::Block &block : region) {
            if (auto terminator = mlir::dyn_cast<mlir::scf::YieldOp>(block.getTerminator())) {
                mlir::ValueRange terminatorOperands = terminator->getOperands();
                rewriter.setInsertionPointToEnd(&block);
                rewriter.create<mlir::BranchOp>(loc, remainingOpsBlock, terminatorOperands);
        ...
        // rewriter.cloneRegionBefore(call->getParentRegion(), rgnval.getRegion(), call->getIterator(), mapping);
